...
| AUTH-03 | ✅ Unit + integration covered. | ❌ No scenario-specific UI test found. | ❌ No scenario-specific E2E test found. | COVERED | Scenario is currently specified at authentication backend contract/rule level; no UI/E2E implementation yet for inactive-user UX. |
| AUTH-04 | ✅ Unit + integration covered. | ❌ No registration UI test found. | ❌ No registration E2E found. | PARTIAL | Registration exists at backend level, but frontend registration journey is not automated yet. |
| AUTH-05 | ✅ Unit + integration covered. | ❌ No duplicate-email UI test found. | ❌ No duplicate-email E2E found. | PARTIAL | Duplicate email rejection is only asserted in backend tests. |
...
| TICKET-USER-02 | ✅ Integration covered. | ❌ No frontend UI scenario test (expected, API security scenario). | ❌ No E2E scenario found. | PARTIAL | Missing browser-level unauthorized create attempt check (without token/session). |
| TICKET-USER-03 | ✅ Integration covered. | ⚠️ `UserTicketsHomePage.test.tsx` tagged `[TICKET-USER-03]`, but relies on backend response fixture and does not prove cross-user leakage by itself. | ❌ No E2E scenario found. | PARTIAL | Frontend assertion is useful but partial for the full isolation claim in the scenario text. |
| TICKET-USER-04 | ✅ Integration covered. | ❌ No ticket-detail ownership UI scenario test found. | ❌ No E2E scenario found. | PARTIAL | Missing UI/E2E proof that owner can open and view full detail. |
| TICKET-USER-05 | ✅ Unit + integration covered. | ✅ `UserTicketsHomePage.test.tsx` tagged `[TICKET-USER-05]` (no status actions visible for user). | ❌ No E2E scenario found. | PARTIAL | UI role restriction exists, but no browser E2E for forbidden status change attempt. |
...
| TICKET-AGENT-02 | ✅ Unit + integration covered. | ❌ No UI test for invalid transition error handling found. | ❌ No E2E invalid-transition scenario found. | PARTIAL | Invalid transition is only validated in backend tests today. |
| TICKET-AGENT-03 | ✅ Integration covered. | ✅ `AgentAdminTicketsPage.test.tsx` tagged `[TICKET-AGENT-03]` + filter/query mapping assertion. | ❌ No E2E scenario found. | PARTIAL | Manageable queue + filtering are in frontend unit tests, but no browser E2E for operational view. |
| TICKET-AGENT-04 | ✅ Unit + integration covered. | ❌ No UI test for “assign to me” action found. | ❌ No E2E scenario found. | PARTIAL | Assignment flow is currently backend-only in automated tests. |
| ADMIN-01 | ✅ Integration covered. | ⚠️ `AdminPage.test.tsx` checks tabs/data rendering, but not deep list semantics for all admin fields. | ❌ No E2E scenario found. | PARTIAL | Basic UI smoke coverage exists; end-to-end admin user-list flow is missing. |
| ADMIN-02 | ✅ Integration covered. | ⚠️ `AdminPage.test.tsx` covers loaded categories at a shallow level. | ❌ No E2E scenario found. | PARTIAL | Needs stronger UI assertions and/or E2E for administrative category listing behavior. |
| ADMIN-03 | ✅ Integration covered. | ❌ No UI deactivation scenario test found. | ❌ No E2E deactivation scenario found. | PARTIAL | User deactivation is validated only via backend automation. |
